...
export default function UltimateBabyBudget() {
  ...
  const projection = useMemo(() => {
    ...
    if (!isCurrentMonth) return totalAmount; // Si mois passé, pas de projection

    const daysPassed = today.getDate();
    const daysInMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0).getDate();
    const dailyAverage = totalAmount / daysPassed;
    return dailyAverage * daysInMonth;
  }, [totalAmount, currentDate, viewScope]);

  // --- ANALYSE COMPARATIVE (M-1) ---
  ...
  }, [expenses, currentDate, viewScope]);
  ...
  const chartDataPoints = useMemo(() => {
    ...
    if (viewScope === 'month') {
      previousMonthExpenses.forEach(e => {
        ...
        mapPrev[key] = (mapPrev[key] || 0) + e.amount;
      });
    }

    return { 
      current: Object.entries(map).map(([l, a]) => ({ label: Number(l), amount: a })).sort((a, b) => a.label - b.label),
      previous: Object.entries(mapPrev).map(([l, a]) => ({ label: Number(l), amount: a })).sort((a, b) => a.label - b.label)
    };
  }, [filteredExpenses, previousMonthExpenses, viewScope]);
  ...
  const lineData = useMemo(() => ({
    ...
    datasets: [
      {
        label: 'Mois Actuel',
        data: Array.from({ length: viewScope === 'month' ? 31 : 12 }, (_, i) => {
          const found = chartDataPoints.current.find(p => p.label === i + 1);
      ...
      ...(viewScope === 'month' ? [{
        ...
        data: Array.from({ length: 31 }, (_, i) => {
          const found = chartDataPoints.previous.find(p => p.label === i + 1);
          return found ? found.amount : 0;
        }),
  ...
  return (
    <div data-theme={theme} className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-[#0B0E14] text-[#F1F5F9]' : 'bg-[#F8FAFC] text-[#0F172A]'}`}
              ...
              <div>
                ...
                {isEditingBudget ? (
                  <div className="flex gap-2">
                    <input type="number" className="bg-[var(--bg-input)] border border-[var(--france-blue)] rounded-lg px-2 py-1 text-xl font-bold w-24" value={tempBudget} autoFocus onChange={(e) => setTempBudget(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && updateBudget()} />
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => { setTempBudget(monthlyBudget.toString()); setIsEditingBudget(true); }} className="text-[10px] opacity-30 hover:opacity-100">MODIFIER</button>
                  </div>
                )}
              </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-[var(--bg-card)] p-8 rounded-[32px] border border-[var(--border)]">
                <h3 className="text-sm mb-6 opacity-70">📈 Comparatif {viewScope === 'month' ? 'M vs M-1' : 'Annuel'}</h3>
                <div style={{ height: '220px' }}><Line data={lineData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: viewScope === 'month' } } }} /></div>
              </div>
              <div className="bg-[var(--bg-card)] p-8 rounded-[32px] border border-[var(--border)]">
            ...
            <div className="bg-[var(--bg-card)] p-10 rounded-[40px] border border-[var(--border)]">
              ...
              <form onSubmit={handleAddExpense} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <button type="submit" disabled={isLoading || profiles.length === 0} className="md:col-span-2 bg-[var(--france-red)] text-white font-black py-5 rounded-2xl hover:brightness-110">AJOUTER</button>
              </form>
            </div>
          </div>
                ...
                {profiles.map(p => (
                  <div key={p.id} className="flex justify-between items-center p-4 bg-[var(--bg-input)] rounded-2xl">
                    ...
                    {pendingDeleteProfileId === p.id ? (
                      ...
                      <button onClick={() => setPendingDeleteProfileId(p.id)} className="text-[10px] opacity-30 hover:text-red-500 font-bold">RETIRER</button>
                    )}
                  </div>
                ))}
              </div>
              <div className="flex gap-2">
                <input className="bg-[var(--bg-input)] p-4 rounded-xl text-sm w-full outline-none" placeholder="Prénom..." value={newProfileName} onChange={(e) => setNewProfileName(e.target.value)} />
            ...
            <div className="space-y-4">
              ...
              <div className="max-h-[500px] overflow-y-auto space-y-3 scrollbar-hide pr-2">
                {filteredExpenses.slice().reverse().map(exp => (
                  <div key={exp.id} className="flex justify-between items-center p-4 bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] group">
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => deleteExpense(exp.id)} className="opacity-0 group-hover:opacity-100 text-red-500">✕</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {toast && (
        <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[2000] px-8 py-4 rounded-2xl shadow-2xl font-bold bg-[var(--france-blue)] text-white`}>
          {toast.msg}
        </div>
      )}
      ...
      <style jsx global>{`
        ...
        @keyframes pulseSuccess {
          0% { background-color: rgba(92, 124, 250, 0.2); }
