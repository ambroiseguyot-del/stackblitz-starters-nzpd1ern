...
const ARTICLES_A_VENIR = [
  ...
  { titre: 'Avoir un bébé à Paris vs province : les vraies différences de coût', categorie: 'Budget & finances', emoji: '🗺️' },
];
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px 80px' }}>
          ...
          <Link key={article.slug} href={`/blog/${article.slug}`} style={{ textDecoration: 'none', display: 'block', marginBottom: 40 }}>
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 99, background: 'rgba(99,102,241,0.2)', color: '#A5B4FC', letterSpacing: '0.06em', textTransform: 'uppercase' }}>À la une</span>
                <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 99, background: 'rgba(255,255,255,0.08)', color: '#64748B' }}>{article.categorieLabel}</span>
              </div>
              ...
              <p style={{ fontSize: 14, color: '#94A3B8', lineHeight: 1.7, marginBottom: 24, maxWidth: 600 }}>{article.description}</p>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {article.sources.map(s => (
                    <span key={s} style={{ fontSize: 11, padding: '3px 8px', borderRadius: 6, background: 'rgba(255,255,255,0.06)', color: '#475569', border: '1px solid rgba(255,255,255,0.08)' }}>📎 {s}</span>
                  ))}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <span style={{ fontSize: 12, color: '#64748B' }}>⏱ {article.lecture} de lecture</span>
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#6366F1' }}>Lire l'article →</span>
                </div>
              </div>
            </div>
          </Link>
        ...
        <div style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: '#0F172A', letterSpacing: '-0.5px' }}>Tous les articles</h2>
              ...
              <Link key={article.slug} href={`/blog/${article.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 99, background: '#EFF6FF', color: '#1D4ED8' }}>{article.categorieLabel}</span>
                  </div>
                  <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0F172A', lineHeight: 1.3, marginBottom: 10, flex: 1 }}>{article.titre}</h3>
                  <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.6, marginBottom: 16 }}>{article.description.substring(0, 120)}...</p>
            ...
            {ARTICLES_A_VENIR.map((article, i) => (
              <div key={i} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 99, background: '#F8FAFC', color: '#94A3B8', border: '1px solid #E2E8F0' }}>{article.categorie}</span>
        ...
        <div style={{ marginTop: 56, background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 20, padding: '32px', textAlign: 'center' }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#1D4ED8', marginBottom: 10 }}>Restez informé</p>
          <h3 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#0F172A', letterSpacing: '-0.5px', marginBottom: 10 }}>Un nouvel article chaque semaine</h3>
